...
$(window).scroll(function () {
    ...
    if ($(this).scrollTop() > 100) {
        if (!menuOpened) {
            $('.start').addClass("follow");
        }
    }
    else {
        $('.start').removeClass("follow");
    }
});
...
function openNav() {
    ...
    $(".hero-home .txt").fadeOut();
    $(".hero-home .slide").css({ "transform": `translateX(-20%)`, "transition": "transform 0.9s ease-in-out" });
    // $("#mainmenu").css('background',"url('"+$("resources/img/")+"')")
    menuOpened = true;
    $(".start").removeClass('follow');
    $("#mainmenu .mainmenu-bg").addClass('opened');
    $("#mainmenu .mainmenu-leftList ul li").each(function (i) {
        let sto = setTimeout(() => $(this).addClass('opened'), 100 * (i + 1))
        //    $(this).delay(120 * (i+1)).fadeIn(400);
    });
    let L;
    $("#mainmenu .mainmenu-rightList ul li").each(function (i) {
        let sto1 = setTimeout(() => $(this).addClass('opened'), 100 * (i + 1))
        //    $(this).delay(120 * (i+1)).fadeIn(400);
        L = i;
    });
    let sto2 = setTimeout(() => $("#mainmenu .lang-selector").addClass('active'), 100 * (L + 1))
    let sto3 = setTimeout(() => $("#mainmenu .social-selector").addClass('active'), 100 * (L + 1))
...
function closeNav() {
    ...
    menuOpened = false;
    $(".hero-home .txt").fadeIn();
    $(".hero-home .slide").css({ "transform": `translateX(0px)`, "transition": "transform 0.9s ease-in-out" });
    $("#mainmenu .mainmenu-bg").removeClass('opened');

    if ($(window).scrollTop() > 100) {
        $(".start").addClass('follow');
    }
    let l = $("#mainmenu .mainmenu-leftList ul li").length;
    $("#mainmenu .mainmenu-leftList ul li").each(function (i) {
        //    $(this).delay(60 * (l-i)).fadeOut(200);
        let sto = setTimeout(() => $(this).removeClass('opened'), 60 * (l - i))

    });

    let l1 = $("#mainmenu .mainmenu-rightList ul li").length;
    $("#mainmenu .mainmenu-rightList ul li").each(function (i) {
        //    $(this).delay(60 * (l-i)).fadeOut(200);
        let sto1 = setTimeout(() => $(this).removeClass('opened'), 60 * (l1 - i))

    });
    $("#mainmenu .lang-selector").removeClass('active');
    $("#mainmenu .social-selector").removeClass('active');
...
function closeAndGo() {
    ...
    $("#mainmenu .blackBase").velocity({ opacity: 0 }, 600);


}
$(function () {
    let menu = $("#mainmenu");
    fetch((window.MAINMENU_URL||'mainmenu.html')).then((d) => {
        return d.text();
    }).then(txt => {
        menu.html(txt);
        ...
        menu.find("a[href]").click(function (e) {
           // if (e) e.preventDefault();
            //closeAndGo();
            //setTimeout(() => window.location.href = $(this).attr('href'), 700);


        });

        if ($(".hero-home .slide img").length > 0) {
            menu.find(".mainmenu-bg img").attr("src", $(".hero-home .slide img").eq(0).attr('src'))
        }
    });
...
if(imgs==null || imgs==undefined){
    ...
    let fi = hero.find(".slide img").eq(0);
     fi.css({'position':"absolute","opacity":0.4});
    /*waitLoad(fi, (img) => {
    ...
    fi.animate({ opacity: 0.4 }, VEL, function () {
        sto = setTimeout(next, TIME);
    ...
    function showActual() {
        ...
        waitLoad(nimg, function (im) {
            nimg.animate({ opacity: 0.4 }, VEL);

            hero.find(".slide .toRemove").animate({
                opacity: 0
            }, VEL, function () {
                $(this).remove();
                active = false;
...
$(function () {
    $('.testimonials').each(Testimonials);

    function Testimonials() {
        let div = $(this);
        let txts = div.find('.txt').clone();

        let n = txts.length;
        if (n < 2) return;
        let index = 0;
        let sto;
        let dvh4 = div.find('.txt h4').eq(0);
        let dvauth = div.find('.txt .author').eq(0);
        txts.each(function () {
            $(this).find("h4").removeAttr('data-aos')
            $(this).find(".author").removeAttr('data-aos')

            $(this).find("h4").removeAttr('data-aos-duration')
            $(this).find(".author").removeAttr('data-aos-duration')

        });

        dvh4.removeAttr('data-aos');
        dvh4.removeAttr('data-aos-duration');
        dvauth.removeAttr('data-aos');
        dvauth.removeAttr('data-aos-duration');

        div.find('.txt').each(function (i) {
            if (i != 0) $(this).remove();
        })
        ...
        function next() {
            index++;
            if (index >= n) index = 0;
            let nw = txts.eq(index).clone();


            dvauth.velocity({ opacity: 0 }, 200, function () {
                dvauth.html(nw.find('.author').html());
            });

            dvh4.velocity({ opacity: 0 }, 500, function () {
                dvh4.html(nw.find('h4').html());
                dvh4.velocity({ opacity: 1 }, 600, function () {
                    dvauth.velocity({ opacity: 1 }, 400, function () {
                        sto = setTimeout(next, 4000);
                    });
                });
...
(function(){
    ...
    function calcTime(city, offset) {
       ...
       console.log(nd.toLocaleString());
        let str=nd.toLocaleString().split(',')[1];
        let pm=str.split(" ");
        pm=pm[pm.length-1];
    let str1=str.split(":");
    let h=str1[0].trim();
    let m=str1[1].trim();
    if(h<10)h=`0${Number(h)}`;
    if(m<10)m=`0${Number(m)}`;
    return h+" : "+m+" "+pm;
    }
    
    let str=calcTime('Mexico', mexico.offset);
     let hdiv=document.querySelector("footer .hora");
     if(hdiv){
         hdiv.innerHTML=str;
        }
        ...
        setInterval(()=>{
            str=calcTime('Mexico', mexico.offset);
            if(hdiv){hdiv.innerHTML=str;}
    ...
    let hhdiv=document.querySelector("footer .hora");
    if(hhdiv){
     let td=new Date();
     hdiv.innerHTML=`${td.getHours()<10?"0"+td.getHours():td.getHours()} : ${td.getMinutes()<10?"0"+td.getMinutes():td.getMinutes()} `;
    }
    })();
    } 
