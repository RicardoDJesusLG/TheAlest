...
$twig = new \Twig\Environment($loader);
    ...
    // articles
    $tableId = 'id';
    $query = $conn->prepare("select * from {$config['prefix']}articles where slug = :slug and lang = '{$langSuffix}' and status = '1'");
    ...
    if (count($res)) {
        foreach ($res as $row) {
            ...
            $render['intro'] = $row['intro'];
            $render['text'] = $row['description'];
            $render['image'] = $config['site_url'].'/files/articles/'.$row['image'];
            
            $render['link'] = getArticleLink($config, $row['slug']);
