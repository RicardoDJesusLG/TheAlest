...
$twig = new \Twig\Environment($loader);
    ...
    // articles
    $tableId = 'id';
    $query = $conn->prepare("select * from {$config['prefix']}articles where slug{$langSuffix} = :slug and status{$langSuffix} = '1'");
    ...
    if (count($res)) {
        foreach ($res as $row) {
            ...
            $render['date'] = date("M d, Y", strtotime($row['pubDate']));
            $render['title'] = $row['title'.$langSuffix];
            $render['intro'] = $row['intro'.$langSuffix];
            $render['text'] = $row['description'.$langSuffix];
            $render['image'] = $config['site_url'].'/files/articles/'.$row['image'];
            
            $render['link'] = getArticleLink($config, $row['slug'.$langSuffix]);
        ...
        header("location: ".$render['link_news']);
        exit;
    }
