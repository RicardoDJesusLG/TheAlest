...
$tableName = "{$config['prefix']}specials";
$tableId = "id";
$orderBy = "position";
// ------------------------------------------------- //
...
$twig = new \Twig\Environment($loader);
	...
	if ($render['mode'] == 'list') {
		...
		$images = array('image' => array('validate' => true, 'uploadDir' => '../files/specials'));
		if ($_POST['submitUpdate']) {
			...
			if (!$errorMsg) {
				...
				foreach ($config['siteLangs'] as $langSuffix) {
					$fields['status'.$langSuffix] = abs($fields['status'.$langSuffix]);
				}
