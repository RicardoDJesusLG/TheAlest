...
$twig = new \Twig\Environment($loader);
	...
	if ($render['mode'] == 'list') {
		...
		$images = array('image' => array('validate' => true, 'uploadDir' => '../files/rooms'), 'servicesImage' => array('validate' => true, 'uploadDir' => '../files/rooms'));
		if ($_POST['submitUpdate']) {
			...
			foreach ($config['siteLangs'] as $langSuffix) {
				...
				$validate[] = 'intro2'.$langSuffix;
				$validate[] = 'text'.$langSuffix;
				$validate[] = 'pretitle'.$langSuffix;
				$validate[] = 'services'.$langSuffix;
			}
			...
			foreach ($config['siteLangs'] as $langSuffix) {
				$validate[] = 'specialInfoTitle'.$langSuffix;
				$validate[] = 'specialInfoText'.$langSuffix;
			}
		...
		$tableIdTmp = "id";
	    unset($items);
		$query = $conn->prepare("select * from {$config['prefix']}rooms_types");
		$query->execute();
		$res = $query->fetchAll();
		if (count($res)) {
			foreach ($res as $row) {
				...
				$items[$row[$tableIdTmp]]['text'] = $row['titleEn'];
				$items[$row[$tableIdTmp]]['id'] = $row[$tableIdTmp];
				$items[$row[$tableIdTmp]]['selected'] = $row[$tableIdTmp] == $array['idType'] ? 'selected' : '';			
			}

			$render['idTypes'] = $items;
		}
