...
$twig = new \Twig\Environment($loader);
    ...
    // aabb
    $tableId = 'id';
    $query = $conn->prepare("select * from {$config['prefix']}aabb where slug = :slug");
    ...
    if (count($res)) {
        ...
        foreach ($res as $row) {
            ...
            if ($row['reservations'.$langSuffix])
                $render['reservations'] = $row['reservations'.$langSuffix];

            $render['link'] = getLinkAABB($config, $row[$tableId]);
        ...
        header("location: ".$render['link_aabb']);
        exit;
    }
