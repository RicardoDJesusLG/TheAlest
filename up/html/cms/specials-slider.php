...
// ------------------------------------------------- //
$sectionTitle = "Specials > Slider";
$sectionMenu = "specials";
$tableName = "{$config['prefix']}specials_slider";
$tableId = "id";
$orderBy = "position";
// ------------------------------------------------- //
	...
	if ($render['mode'] == 'list') {
		...
		if (count($res)) {
			...
			foreach ($res as $row) {
				...
				$items[$row[$tableId]]['image'] = '../files/slider/thumbs/'.$row['image'];
		...
		$images = array('image' => array('validate' => true, 'uploadDir' => '../files/slider', 'thumbs' => array(0 => array('uploadDir' => '../files/slider/thumbs', 'width' => '100', 'height' => '100'))));
