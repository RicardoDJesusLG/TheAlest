...
// ------------------------------------------------- //
$sectionTitle = "Rooms & Suites > Slider";
$sectionMenu = "rooms";
$tableName = "{$config['prefix']}rooms_slider";
$tableId = "id";
$orderBy = "position";
// ------------------------------------------------- //
...
// related
$tableIdRelated = "id";
$rowRelated = xmlFormat(getArray($config, $tableIdRelated, "{$config['prefix']}rooms", $_GET['related']));
if (!$rowRelated[$tableIdRelated]) {
	header("location: ./");
	exit;
}
...
if ($_POST['submitDelete']) {
	...
	header("location: ?msgUpdated=1&related=".$rowRelated[$tableIdRelated]);
	exit;
}
...
$twig = new \Twig\Environment($loader);
	...
	$render['nameRelated'] = $rowRelated['titleEs'].' / '.$rowRelated['titleEn'];
	$render['idRelated'] = $rowRelated[$tableIdRelated];
	...
	if ($render['mode'] == 'list') {
		...
		$items = array();
		$query = $conn->prepare("select * from $tableName where idRelated = {$render['idRelated']} order by $orderBy");
		$query->execute();
		$res = $query->fetchAll();
		...
		if (count($res)) {
			...
			foreach ($res as $row) {
				...
				$items[$row[$tableId]]['title'] = $row['titleEn'].' / '.$row['titleEs'];
		...
		$render['linkAdd'] = $render['currentPage'].'?mode=edit&related='.$render['idRelated'];

	} elseif ($render['mode'] == 'edit') {

		$images = array('image' => array('validate' => true, 'uploadDir' => '../files/rooms'));
		...
		if ($_POST['submitUpdate']) {
			...
			if (!$errorMsg) {
				...
				$fields['idRelated'] = $render['idRelated'];
		...
		$render['linkCancel'] = $render['currentPage'].'?related='.$render['idRelated'];
	}
